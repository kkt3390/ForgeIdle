...
function updateManualHuntButton() { if (!state) return; const at = manualHuntAvailableAt || state.manualHunt.availableAt; const remaining = at ? Math.max(0, new Date(at).getTime() - Date.now()) : 0; $("#manual-hunt-button").disabled = remaining > 0; $("#manual-hunt-button").textContent = remaining > 0 ? `${Math.ceil(remaining / 1000)}초 후 가능` : "몬스터 처치"; $("#manual-hunt-cooldown").textContent = remaining > 0 ? "다음 몬스터를 찾고 있습니다." : state.hunt ? "클릭하면 자동 사냥을 정산하고 직접 사냥으로 전환합니다." : "3초마다 직접 처치할 수 있습니다."; }
