...
SET Nickname = JSON_VALUE(StateJson, N'$.Nickname'),
    ...
    DestructionResistance = COALESCE(TRY_CONVERT(int, JSON_VALUE(StateJson, N'$.Stats.DestructionResistance')), DestructionResistance),
    RecoveryMastery = COALESCE(TRY_CONVERT(int, JSON_VALUE(StateJson, N'$.Stats.RecoveryMastery')), RecoveryMastery)
...
BEGIN
    ...
    CREATE INDEX IX_ea_admin_action_logs_CreatedAt
        ON dbo.ea_admin_action_logs (CreatedAt DESC);
    CREATE INDEX IX_ea_admin_action_logs_TargetPlayerKey
        ON dbo.ea_admin_action_logs (TargetPlayerKey);
END;
GO
...
BEGIN
    ...
    CREATE UNIQUE INDEX IX_ea_monster_catalog_MonsterKey
        ON dbo.ea_monster_catalog (MonsterKey);
END;
GO
...
BEGIN
    ...
    CREATE UNIQUE INDEX IX_ea_weapon_catalog_WeaponKey
        ON dbo.ea_weapon_catalog (WeaponKey);
END;
GO
